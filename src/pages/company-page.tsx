...
export default function CompanyPage() {
  ...
  const handleCertificateChange = (event: { preventDefault: () => void; target: { files: FileList | null } }) => {
    ...
    if (event.target.files !== null) {
      ...
      dispatch(setCredentialsAttribute({ attribute: "NombreCertificado", value: file.name }));
      reader.onloadend = () => {
        ...
        <Grid container spacing={{ xs: 1, sm: 2 }}>
          ...
          <Grid item xs={12}>
            <TextField
              required
              id="LeyendaProforma"
              value={company.LeyendaProforma}
              label="Leyenda Proforma"
              onChange={handleChange}
            />
          </Grid>
          <Grid item xs={12}>
            <TextField
              required
              id="LeyendaApartado"
              value={company.LeyendaApartado}
              label="Leyenda Apartado"
              onChange={handleChange}
            />
          </Grid>
          <Grid item xs={12}>
            <TextField
              required
              id="LeyendaOrdenServicio"
              value={company.LeyendaOrdenServicio}
