...
function ProductPage({
  ...
  React.useEffect(() => {
    const calculatePrice = (value, taxId) =>
      roundNumber(getPriceFromTaxRate(value, rentTypeList.find(elm => elm.Id === taxId).Valor, false), 2);
  ...
  const handlePriceChange = event => {
    const untaxPrice = roundNumber(
      getPriceFromTaxRate(event.target.value, rentTypeList.find(elm => elm.Id === product.IdImpuesto).Valor, false),
      2
    );
  ...
  const handleUntaxPriceChange = event => {
    const taxPrice = roundNumber(
      getPriceFromTaxRate(event.target.value, rentTypeList.find(elm => elm.Id === product.IdImpuesto).Valor, true),
      2
    );
      ...
      <Grid container spacing={2}>
        <Grid item xs={12}>
          <FormControl fullWidth>
            ...
            <Select id="Tipo" value={product.Tipo} onChange={event => setProductAttribute("Tipo", event.target.value)}>
              {productTypes}
            </Select>
          </FormControl>
        </Grid>
