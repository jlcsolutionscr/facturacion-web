...
function StepFourScreen({
  ...
  const handleOnPrintClick = () => {
    if (status === "converted") {
      ...
      generateWorkingOrderTicket(orderId);
    }
  };
  ...
  return (
    <div ref={myRef} className={classes.container} hidden={value !== index}>
      <Grid container spacing={2} className={classes.gridContainer}>
            ...
            <Grid item xs={12} sm={7} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  id="CodigoActividad"
                  value={activityCode}
                  onChange={event => setOrderAttributes({ activityCode: event.target.value })}
                >
                  {activityItems}
                </Select>
              </FormControl>
        ...
        {orderId === null && vendorItems.length > 1 && (
          <Grid item xs={12} className={classes.centered}>
            <Grid item xs={12} sm={7} md={6}>
              <FormControl fullWidth>
        ...
        {status === "on-progress" && (
          <Grid item xs={12} className={classes.centered}>
            <Button
              ...
              label={orderId !== null ? "Actualizar" : "Agregar"}
          ...
          <Grid item xs={12} className={classes.centered}>
            <FormControl style={{ width: "215px", textAlign: "left" }}>
              ...
              <Select
                id="forma-Pago"
                value={paymentId}
                onChange={event => setOrderAttributes({ paymentId: event.target.value })}
              >
                {paymentItems}
              </Select>
            </FormControl>
...
const mapStateToProps = state => {
  return {
    orderId: state.workingOrder.orderId,
    orderConsec: state.workingOrder.orderConsec,
    cashAdvance: state.workingOrder.cashAdvance,
