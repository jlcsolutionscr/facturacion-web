...
function ListDropdown({page, rowsCount, rowsPerPage, rows, label, value, disabled, onChange, onPageChange, onItemSelected}) {
  ...
  return (<div id='main-container' tabIndex='1' className={classes.container} onClick={(e) => e.stopPropagation()}>
    <div id='input-container' className={classes.root}>
      <input
        ...
        onChange={onChange}
      />
      <label id='main-container' className={`${classes.label} ${classes.font} ${open && classes.labelOutline}`}>{label}</label>
    </div>
    <div
      id='items-container'
    ...
    >
      ...
      {rowsCount > rowsPerPage && <div style={{width: '100%'}}>
        <TablePagination
